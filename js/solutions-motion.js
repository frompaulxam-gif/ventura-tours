import * as THREE from '../concepts/connected-v2/vendor/three.module.min.js';
import {GLTFLoader} from '../concepts/flow/vendor/GLTFLoader.js';
import {environment} from '../concepts/connected-v2/assets.js?v=4';
...
$('#motion-toggle').addEventListener('click',()=>{paused=!paused;document.dispatchEvent(new CustomEvent('ventura-motion',{detail:{paused}}));$('#motion-toggle').setAttribute('aria-pressed',String(paused));$('#motion-toggle').textContent=paused?'Enable motion ▷':'Reduce motion Ⅱ';mode()});
...
async function build(){
 ...
 const gltf=await new GLTFLoader().loadAsync(new URL('../concepts/signal-studio/assets/ventura-ai-workflow.glb?v=1',import.meta.url).href);
 ...
 let lastW=0,lastH=0;resizeScene=()=>{const w=visual.clientWidth,h=visual.clientHeight;if(!w||!h||(w===lastW&&h===lastH))return;lastW=w;lastH=h;renderer.setPixelRatio(Math.min(devicePixelRatio||1,mobile.matches?1.3:1.5,Math.sqrt(1100000/(w*h))));renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();fit=Math.min(1,viewHeight*camera.aspect/(mobile.matches?6.2:11.6));layout()};
 render=p=>{
  ...
  labels.forEach((label,i)=>{point.copy(slots[i]);point.y-=1.38;point.applyMatrix4(world.matrixWorld).project(camera);label.style.left=`${(point.x*.5+.5)*lastW}px`;label.style.top=`${(-point.y*.5+.5)*lastH}px`;label.style.opacity=range(p,.36,.48)});
  renderer.render(scene,camera)
 };
 canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();fallback('WebGL context lost')});resizeScene();new ResizeObserver(()=>{resizeScene();request()}).observe(visual);renderer.compile(scene,camera);document.body.classList.add('enhanced');mode();measure();progress=target;render(progress);paint(progress);document.body.classList.add('ready');request();
}
