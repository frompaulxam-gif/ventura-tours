'use strict';
const details = {
 living: {title:'Living space',image:'assets/living.jpg',text:'Original timber, cast iron columns and a place to settle in. This is a published photo of Jason Works Loft.',source:'https://www.jasonworksloft.com/'},
 dining: {title:'Dining & gathering',image:'assets/dining.jpg',text:'A closer look at the loft for dinners and gatherings. Ask the venue about a layout for your event.',source:'https://www.jasonworksloft.com/events'},
};
...
function openDetail(id){
 ...
 content.innerHTML=`<img class="detail-image" src="${d.image}" alt="Published venue photo: ${esc(d.title)}"><div class="detail-copy"><div class="eyebrow">Original reference photo</div><h2>${esc(d.title)}</h2><p>${esc(d.text)}</p><p class="small">Photo credit: Jason Works Loft website. Included as a reference for this concept.</p><a class="enquire" href="${d.source}" target="_blank" rel="noopener">View venue information ↗</a></div>`;
 if(!dialog.open)dialog.showModal();
}
function about(){content.innerHTML='<div class="detail-copy"><div class="eyebrow">Made by Ventura Tours</div><h2>A first look at what is possible.</h2><p>This sample pairs an AI generated 360° concept with original photos from the Jason Works Loft website.</p><p>The two viewpoints are AI interpretations, linked to suggest moving through the loft. Layout, finishes and unseen areas may differ. It is not a measured scan or a record of the current venue.</p><p>A finished tour would use real 360° photography captured onsite, with room links and your own booking or enquiry buttons.</p><p class="small">An independent proposal for discussion. Not an official Jason Works Loft tour. Original photos remain the property of their respective rights holders.</p><a href="https://www.jasonworksloft.com/" target="_blank" rel="noopener">Original venue website ↗</a></div>';if(!dialog.open)dialog.showModal();}
...
let viewer,rotating=false,moving=false,ready=false,current='living';
const reducedMotion=window.matchMedia('(prefers-reduced-motion: reduce)');
const viewpoints={
 living:{image:'assets/loft qc.png',yaw:-30,pitch:-5,label:'01 / Living space',heading:'Beside the sofas.',links:[{id:'dining',yaw:94,pitch:-16}]},
 dining:{image:'assets/dining qc.png',yaw:95,pitch:-5,label:'02 / Dining & gathering',heading:'At the dining table.',links:[{id:'living',yaw:-90,pitch:-15}]},
};
...
function updatePlace(){
 ...
 document.querySelector('#help').textContent='Select an arrow to move. Drag to look around.';
...
async function goTo(id){
 ...
 setMoving(true);
 document.querySelector('#help').textContent='Moving to '+details[id].title.toLowerCase()+'…';
 ...
 viewer.lookAt(pitch,yaw,hfov,duration,()=>viewer.loadScene(id,pitch,yaw,hfov));
}
...
try {
 const scenes=Object.fromEntries(Object.entries(viewpoints).map(([id,p])=>[id,{type:'equirectangular',panorama:p.image,pitch:p.pitch,yaw:p.yaw,hotSpots:p.links.map(l=>({pitch:l.pitch,yaw:l.yaw,cssClass:'spot',createTooltipFunc:marker,createTooltipArgs:{id:l.id,label:details[l.id].title}}))}]));
 viewer=pannellum.viewer('panorama',{default:{firstScene:'living',autoLoad:true,showControls:false,mouseZoom:false,hfov:92,minHfov:50,maxHfov:110,escapeHTML:true,sceneFadeDuration:reducedMotion.matches?0:1100},scenes});
